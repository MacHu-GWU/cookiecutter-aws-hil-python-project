...
def is_hil_branch(git_branch: str) -> bool:
    return git_branch.lower().startswith("hil")
...
IS_HIL_BRANCH: bool = is_hil_branch(GIT_BRANCH_NAME)
...
COMMIT_MESSAGE_HAS_HIL: bool = False
COMMIT_MESSAGE_HAS_LBD: bool = False
if IS_CLEAN_UP_BRANCH:
    commit_parser = ConventionalCommitParser(types=["cf", "hil", "lbd"])
    has_cf_commit = is_certain_semantic_commit(
        GIT_COMMIT_MESSAGE, stub="cf", parser=commit_parser
    )
    has_hil_commit = is_certain_semantic_commit(
        GIT_COMMIT_MESSAGE, stub="hil", parser=commit_parser
    )
